...
SimulationBody::SimulationBody(std::string name, double mass, Vect2 position, Vect2 velocity, Vect2 force,
    double kinetic_energy, double potential_energy, sf::Color color, float radius) :
    name(name), mass(mass), position(position), velocity(velocity), force(force),
    kinetic_energy(kinetic_energy), potential_energy(potential_energy),
    color(color), drawRadius(radius) {};
