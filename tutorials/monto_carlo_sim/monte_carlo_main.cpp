...
//    Node * current = scene->shapes.get();
    ...
    for (auto& shape : scene->shapes) {
        shape.x += step_size* getRandom(generator, distribution);
        if (shape.x > x_max) {
            std::cout << "Adjusting for boundary at x max" << std::endl;
            shape.x -= x_max;
        }
        if (shape.x < x_min) {
            std::cout << "Adjusting for boundary at x min" << std::endl;
            shape.x += x_max;
        }
        shape.y += step_size * getRandom(generator, distribution);
        if (shape.y > y_max) {
            std::cout << "Adjusting for boundary at y max" << std::endl;
            shape.y -= y_max;
        }
        if (shape.y < y_min) {
            std::cout << "Adjusting for boundary at y min" << std::endl;
            shape.y += y_max;
        }
    }
